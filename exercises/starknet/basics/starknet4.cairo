...
mod LizInventory {
    ...
    impl LizInventoryImpl of super::ILizInventory<ContractState> {
        fn add_stock(ref self: ContractState, product: felt252, new_stock: u32) {
            ...
            let currentStock = self.inventory.read(product);
            self.inventory.write(product, new_stock + currentStock);
        }
        ...
        fn purchase(ref self: ContractState, product: felt252, quantity: u32) {
            ...
            let currentQty = self.inventory.read(product);
            assert(currentQty >= quantity, 'Required qty exceeds stock');
            self.inventory.write(product, currentQty - quantity);
        }
